...
authoring   draft(machine) ──▶ refining(machine) ──▶ backlog(human)
                                                          ...
                                                          ▼
execution   ready(machine) ──▶ [decomposing(machine) ──▶ awaiting-children(none) ──▶ evaluating(machine)]  ──▶ adjudicating(machine) ──▶ done(none)
              │                 └─ 仅 compound（Epic）走这段；primitive：ready ──▶ evaluating ─┘              │
              └──▶ needs-human(human) ──▶ ready | done                                                       └──▶ needs-human(human)  // 或单步回退到 task.entry_phase（BACK-682，见 retreat.ts）
...
- **`adjudicating`（BACK-682）**：ENG-8 机械 DoD 判定（`adjudicate()`，`src/engine/
  adjudicate.ts`，签名/行为不变）给出 `done` 时，execution pipeline 并不直接落
  `done`——而是先落 `adjudicating`，一个独立的、judgment-based 复核（不是重跑 DoD）。
  该 phase 只有两种去处：确认 → `done`/`needs-human`（`completeAdjudication`），或发现
  spec/decomposition/goal 层的缺口 → 单步回退到 `task.entry_phase`（`recordRetreat`，
  `src/engine/retreat.ts`；带 gap-fingerprint 去重与三段式 keep/missing/wrong 契约，
  永不跨级、永不二次退同一缺口)。实现层缺口不走这条路——留在 primitive-executor 内循环
  自行修正。
...
| execution / adjudicating | machine | `adjudicate` | **extract** ← `src/engine/retreat.ts`（独立 judgment 复核 + 单步回退契约） | BACK-682 |
