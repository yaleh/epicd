---
adr: "012"
title: "从 baime 原型迁移的运行时不变量：ENG-6 场身份 / ENG-7 落盘原子合法 / ENG-8 无自我认证终态"
status: Proposed
date: 2026-07-03
applies-to:
...
**Date**: 2026-07-03
**Deciders**: Yale Huang
**Ports / distills**: baime `loop-backlog` / `loop-draft` 在 2026-07-02～07-03 窗口实测出的运行时失败比特；并移植 baime ADR-017（后台 Agent status 写权限的两层信任模型）
...
| implementer Agent 自写终态 `Done` = 对自己刚完成的工作自我认证，摧毁独立复核（baime ADR-017 两层信任模型） | worker 兼任执行者与终态裁决者 | **ENG-8** |
...
#### ENG-8: 无自我认证终态 —— worker 不得自写 `done`，引擎第三方复核后裁决（移植 baime ADR-017）

- 产出**被 merge 的工件**的 worker（implementer 类）**MUST NOT** 自行把 Task 写成成功终态
  （`done`）。终态是一道合并/DoD 复核关卡，写权不属于产出者。
- 终态裁决由**引擎自身**（独立第三方）在 `engine.complete(taskId, result)` 回程、**merge 锁下
  重新验证 DoD** 后写入：任一 DoD 不过 → `needs-human`，**绝不** `done`。这把 baime 的
  `complete-task.sh`（merge 前于 worktree 内独立重跑每条 DoD shell-gate）落成引擎 core 的一等职责。
- worker 只能追加进度/结果（`engine.complete` 的 `result` payload），**不持有** `state→done` 的写权。
- **例外（decomposer 类）**：产出**纯规划工件**（子 Task）、无 merge、无需第三方合并前复核的
  handler，**MAY** 自写自身工作流状态（如 `awaiting-children`），由自身 guard 自校验——对应
  baime ADR-017 的 decomposer 豁免。判据："这个 handler 的产出是否需要独立第三方在合并/落地前
  复核？"需要 → implementer 模型（result-only，引擎写终态）；不需要 → decomposer 模型（自写自校验）。
- **与 ENG-1/ENG-3 的关系**：ENG-1 保证状态推进先于 spawn、ENG-3 保证 merge 串行 + 冲突即停；
  ENG-8 补上二者未明说的一面——**"谁有权写终态 `done`"**：不是 worker，是重跑 DoD 的引擎。
- **删除的非法态**：worker 自我认证刚完成的工作为 `done`；未过 DoD 的工件进成功终态。
...
- **归属 Epic**：ENG-6 归 **BACK-600.4**（驱动器 detect→spawn→merge→advance 必须以 `field`
  ...
  **BACK-600.4 + 捕获/authoring 入口**（对应本仓 `task-1` authoring-as-pipeline）；ENG-8 归
  **BACK-600.4（`engine.complete` 终态裁决）+ BACK-600.5（merge 锁下重跑 DoD）**。
- **测试载体**（待 BACK-600.4 驱动器落地后建立）：
  ...
  - `src/test/engine-no-self-certify.test.ts`：断言 worker 的 `result` 不能把 Task 推成 `done`；
    仅当引擎重跑 DoD 全过才 `done`，任一不过 → `needs-human`；decomposer 类可自写 `awaiting-children`。
  - transition 自清：逐 transition 的单测（随 ADR-011 D-2 `transitions[]` 实现一并建立）。
- E0 是否强制 ENG-6/7/8 全量：ENG-6 的"单例/场身份"随 BACK-600.4 驱动器同期落地（否则驱动器
  一多就损坏）；ENG-7 的"原子创建"随第一个写盘入口落地；ENG-8 随 `engine.complete` + merge 锁
  落地（BACK-600.4/600.5）。ENG-7 附则待 `transitions[]` 存在时生效。
...
- 与 ADR-010 的关系：ADR-010 是 fork **之前**的 15 条；本 ADR 是 fork **之后** baime 原型
  在多 scanner + 捕获流水线机制下**新实测**的补充，外加移植 baime ADR-017 的信任模型。
  三者同属"别把库改坏 + 别让并发损坏状态 + 别让产出者自我认证"的安全栏，编号连续
  （ENG-1…5 in ADR-010，ENG-6…8 here）。
- ENG-8 把 baime 的 `complete-task.sh` 独立复核角色，从一个外部 shell 脚本提升为引擎 core
  的一等职责：终态写权集中在重跑 DoD 的引擎，与 ENG-3 的 merge 裁决同址，堵死 worker 自证。
...
- baime ADR-017（两层信任模型，ENG-8 出处）：`/home/yale/work/baime/docs/adr/ADR-017-background-agent-status-authority.md` + `complete-task.sh` 独立复核机制
