...
| 4 | BACK-655（任务生命周期模型 conformance：backfill 选对 pipeline + task create --pipeline/--phase + phase 合法性校验 + drift-lint） | 2026-07-06 | ~75 分钟 | 2——两次均为**操作性 needs-human**，但根因与前 3 例都不同：`engine complete --worktree` 的 board-only-conflict 自动解冲突逻辑（`gitMergeBranch`，`src/harness/real-primitives.ts`）用 `git diff --name-only`（无 `-z`）取待解冲突路径，遇到本任务这种含中文字符的 board 文件名时该输出被 git 转义/加引号，后续 `git checkout --ours`/`git add` 的 pathspec 因此匹配不到真实文件，解冲突静默失败、回退为 needs-human——这是一个此前 3 个样本都没撞上的**新根因类别**（非缺 dodGates、非 adjudicate/dodResults 不一致），已独立归档为 BACK-662（高优先级，因为几乎所有真实 task 标题都含中文，理论上会反复复现）；人工手动重跑 `-z` 版本验证可干净解决后，手动完成本次合并（`87fa675`）并独立复核全部 7 条结构化 DoD gate | ①✓声明 dodGates ②✗未走通 `engine complete` 自动合并路径（本质是合并机制操作性缺陷，非 dodGates 缺失、非合法安全默认值）③✓审计决策显式执行（独立 fresh-context 审计 agent，发现 1 处中等严重度缺口，已归档 BACK-661）④✓**没有新增 forced step**——沿用既有的根因分类/人工收口/审计三步 → **不合格（判据②不达标），但④再次为"零新增步骤"** | 不变 | **否**——第二个零新增步骤样本，但因②未达标，仍不能计入"合格 streak" | Basic: Done（人工收口——因 `engine complete` 合并机制自身的中文文件名 quoting 缺陷，非本任务代码问题） |

**当前 streak 状态：0。** 四个历史样本没有一个同时满足全部 4 条合格
判据——BACK-649/654 都因未声明 dodGates 而未走通自动合并分支，BACK-653
虽然全部达标却在过程中新增了 forced step（判据④要求"不新增"），BACK-655
声明了 dodGates 却撞上一个全新根因类别的 `engine complete` 合并机制缺陷
（BACK-662），同样未走通自动合并分支。四个样本恰好轮流卡在四条判据的
不同组合上，没有一个样本四条全过——这本身是一个观察结果：**"合格样本"
比预想的更难凑齐，不是骨架设计的问题，而是自动合并路径（`engine
complete --worktree` 的 board-only-conflict 解冲突）本身还不够健壮**。
skeleton 分项（声明 dodGates、根因分类、审计决策二选一）连续两轮
（BACK-654、BACK-655）保持"零新增步骤"，这仍是骨架已趋稳定的正面信号；
但"合格 streak"的瓶颈已经从"骨架设计不完善"转移到"依赖的自动合并机制
（`engine complete --worktree`）本身有真实缺陷"——BACK-662 修复前，任何
含非 ASCII 字符标题的任务撞上 board-only 冲突时都会重复本次失败模式。
**下一步候选**：BACK-656（已排队，需声明 dodGates）；且 BACK-662 修复后
应优先重跑一个新样本以检验判据②是否能借此真正跑通。
...
**未收敛。当前合格样本 streak = 0**（4 个历史样本均不满足全部 4 条
合格判据——BACK-649/654 因未声明 `dodGates` 未走通自动合并分支，
BACK-653 虽全部达标但过程中新增了 forced step，BACK-655 声明了
`dodGates` 却撞上 `engine complete --worktree` 合并机制自身的中文
文件名 quoting 缺陷（BACK-662）而未走通自动合并分支）。需要 K=3 个
连续满足全部 4 条判据的样本。下一批候选：BACK-656（需在创建/认领时
声明结构化 `dodGates`）；BACK-662 修复后应优先安排一个新样本，以检验
判据②（走通自动合并路径）能否借此真正跑通——目前 4/4 样本都因不同
原因卡在这一条上，是当前最大的单点瓶颈。
