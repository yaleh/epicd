...
| 3 | BACK-654（修复 BACK-649/BACK-653 共同撞上的 adjudicate/dodResults 不一致缺陷本身） | 2026-07-06 | ~50 分钟 | 1——`engine complete --worktree` 判定 needs-human，但这次是**真实门禁生效**而非操作失误：根因调查确认 `adjudicate()`/`completeTask()`/`runDoD()`（TS 路径）本身正确，真正分叉在 `plugin/scripts/complete-task.sh`（shell 路径）把渲染出的人类可读 prose "Definition of Done:" 小节当 shell 命令执行，而非读取结构化 `task.dod` gate；needs-human 触发原因是 BACK-654 任务本身未声明结构化 `dodGates`（有意为之的安全默认值），不是缺陷复发。修复后经独立 fresh-context 审计（另一无实现记忆 agent，重跑全部测试+负控验证+对抗性输入构造）确认零新增阻塞项，两处遗留边缘情况（section 名可被 description 冒充、gate 文本内嵌换行可被拆分）归档为 follow-up BACK-656 | Basic: Done（人工收口——需要人工是因为任务本身缺 dodGates，非引擎自动化失效） |
