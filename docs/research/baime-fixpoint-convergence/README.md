...
**这条子路径已独立拆分为自己的实验**：骨架定义、执行记录表、样本复盘
与收敛判据全部迁移至
[../lightweight-fixpoint/README.md](../lightweight-fixpoint/README.md)。
拆分原因：未来大部分工作是原子 Basic task 而非 epic（epic 数量稀少），
轻量路径值得独立于本实验的 epic 迭代节奏单独收敛，并在收敛后经
knowledge-extractor 提炼为独立 skill。本文档自此只保留 epic 级编排
方法论；轻量路径不再在本文档更新。

与完整 BAIME 的差异（有意简化，非遗漏，epic 级路径本身的说明）：
...
轻量路径的执行记录表、样本复盘（BACK-649/BACK-653/BACK-654）与收敛
判据已完整迁移至
[../lightweight-fixpoint/README.md](../lightweight-fixpoint/README.md)，
并在迁移时补充了更严格的合格样本判据（4 条）与 K=3 streak 收敛规则。
本文档不再维护这部分内容，避免与新实验目录出现内容分叉。
