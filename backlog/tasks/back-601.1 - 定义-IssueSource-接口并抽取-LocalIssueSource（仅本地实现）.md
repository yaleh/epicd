...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 存在最小 `IssueSource` 接口，含 list/get/upsert（可含可选 capabilities）
- [x] #2 `LocalIssueSource` 为唯一实现，委托 Core/FileSystem，读写现有 backlog/tasks/*.md 零回归
- [x] #3 接口可承载 E1 field-registry 派生 schema；engine 专用字段在本地实现透传无损
