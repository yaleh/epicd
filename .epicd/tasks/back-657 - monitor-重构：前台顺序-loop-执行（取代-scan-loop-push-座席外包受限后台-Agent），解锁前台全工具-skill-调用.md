...
4. `bun test src/test/epicd-plugin-synthetic-repo.test.ts`（原计划误写为 skill-portability.test.ts，实际以此已存在的 BACK-605.9 测试落实同一约束——不改名新增第二份，遵守单一实现纪律）— **可移植性门（D-7-bis）**：把打包后的 plugin/ 全树（含全部新老 skill）复制进一个全新 scratch repo 并跑通其 propose/promote/run/inbox 链路后，断言全树任意文件不含 "baime"（大小写不敏感），证明发布 skill 无 baime **运行时**依赖；`provenance:` frontmatter 中的源实验/ADR 引用本就不含字面 "baime"，故不会触发误报。
...
6. 端到端 smoke（证明 epic-lifecycle skill 所包的生产 dispatch 路径在装配后仍可驱动真实 epic，而非仅单测桩）：
   ...
   EID=$(bun run cli task create "coverage-smoke-epic" --labels kind:epic --plain | grep -oiP 'back-\d+' | head -1) \
     && bun run cli task edit "$EID" --pipeline-id execution --phase decomposing \
     && bun run cli engine dispatch "$EID" | grep -q '^epic-ready:' \
     && bun run cli task archive "$EID"
   ```

   注：`engine dispatch` 按 `task.pipeline_id`/`task.phase` 分支（`execution/decomposing`→`epic-ready`）。新建 task 现默认落在 `pipeline_id: authoring, phase: draft`（BACK-664.1 起的默认值，与 `kind:epic` 标签无关），故须显式 `--pipeline-id execution --phase decomposing` 才能跳转合法状态（`task create` 无 `--pipeline-id`/`--phase` 参数，只能 create 后 edit）；task_prefix 为 `back`，`--plain` 内 id 多次出现，故 `grep -oiP 'back-\d+' | head -1`；末尾 `task archive` 清理，避免污染看板。
...
BACK-657.3 落地后复查 epic 自身 Integration Acceptance：发现 IA item #4 引用的 src/test/skill-portability.test.ts 从未创建（各 child 的 AC 均未要求它）；D-7-bis 的可移植性约束实际由已存在的 src/test/epicd-plugin-synthetic-repo.test.ts（BACK-605.9）覆盖（复制打包 plugin/ 全树到 scratch repo 后断言零 baime 字面量）。按单一实现纪律，未新建第二份测试，而是修正 IA 文本指向既有测试。

IA smoke test（item#6）修正：task create 现默认落 pipeline_id=authoring/phase=draft（BACK-664.1），与 kind:epic 标签无关；原命令只 --phase decomposing 会因 authoring pipeline 无 decomposing 合法态而报错。已改为显式 --pipeline-id execution --phase decomposing，并本地验证通过（dispatch 打印 epic-ready:<id>）。
<!-- SECTION:NOTES:END -->
