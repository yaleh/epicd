...
### DoD
- [ ] #1 backlog/drafts/ 下无剩余 *.md 文件（readme.md 除外）：[ -z "$(ls backlog/drafts/*.md 2>/dev/null | grep -v readme.md)" ]
- [ ] #2 全部 15 个现存 draft 内容（标题/正文/created）已迁移为 pipeline_id=authoring, phase=draft 的 Task，用 backlog task list 可见且内容零丢失
- [ ] #3 draft CLI 命令族已移除：bun run cli draft list 2>&1 以非零退出或打印 unknown command
...
### DoD
- [ ] #4 DraftsList.tsx 已删除：! test -f src/web/components/DraftsList.tsx
- [ ] #5 prefix-migration.ts 已删除：! test -f src/core/prefix-migration.ts
- [ ] #6 /api/drafts 路由已移除：! grep -q '"/api/drafts"' src/server/index.ts
- [ ] #7 迁移后任务在 Web All Tasks 的 authoring 泳道下正确显示（manual spot-check：DRAFT-16 的内容可在 authoring 泳道找到）
...
### DoD
- [ ] #8 bun test --parallel 全量通过（含迁移新建测试 + 删除的 draft 专属测试）
- [ ] #9 bunx tsc --noEmit 无错误
...
audit skipped: RiskGated(False) — 变更全为删除（net -2360 行），无新逻辑，不触及 src/engine/，tsc clean，1956 tests pass（2 pre-existing parallel flakes）。DRAFT-16 在 worktree branch point 之后创建，未被迁移脚本覆盖，已手动迁移为 BACK-680。
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
废弃 Draft 实体，折叠进 authoring/draft phase。63 文件变更，524 新增，2884 删除（net -2360）。14 个 draft 文件（DRAFT-2..15）由迁移脚本转为 BACK-667..679；DRAFT-16 手动迁移为 BACK-680。CLI draft 命令族、/api/drafts 路由、DraftsList.tsx、prefix-migration.ts 全部删除。All Tasks 页面现可见 authoring 泳道下的前 draft 任务，根因彻底消除。
