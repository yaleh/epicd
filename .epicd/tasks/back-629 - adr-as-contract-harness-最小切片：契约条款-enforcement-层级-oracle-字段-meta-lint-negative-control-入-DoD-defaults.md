...
- [ ] #8 范围收窄（不完备性作为设计，见 docs/research/2026-07-05-fixpoint-driven-development-constraint-set.md §7）：本切片只做 P0/不可逆层的可执行门 + meta-lint 管道，不追全面分类。executable 化对象限定为不可逆守卫（ENG-1/2/3 安全子集、ENG-8 无自证）+ 已实现失败的回归；其余条款保留 semantic 或不入注册表，刻意留给 dogfooding/外部反馈发现。验收不以'约束集完备'为准，以'P0 不可逆层有 executable oracle + negative-control'为准
<!-- AC:END -->
...
<!-- SECTION:NOTES:BEGIN -->
## 范围收窄补记（2026-07-05 §7 翻转后）
本切片刻意**不追全面分类**。可交付约束集 = { P0 不可逆/破坏性守卫 } ∪ { 已实现失败的回归 }，按设计不完备（docs/research/2026-07-05-fixpoint-driven-development-constraint-set.md §7）。
- Phase 1 重分类的 executable 化对象**限定**为不可逆层：ENG-1（cap 幂等）/ENG-2（worktree 隔离）/ENG-3（merge 串行+冲突即停）/ENG-8（无自证终态）。这些是'发布不起'的失败——必须 pre-ship 门控。
- ADR-013 载体律、ADR-016 正交性等**保留 semantic**，不强行 executable 化——它们属判断层，交 architect reviewer / dogfooding 摩擦发现。
- meta-lint 管道（Phase 2）+ negative-control 入 DoD（Phase 3）照旧——管道是可复用基底，与分类规模无关。
- **不做**：为想象中的失败预造约束；把 semantic 层做全；回填历史条款。约束集后续由 dogfooding 已实现摩擦驱动沉积（memory: dogfooding-friction-over-internal-audit），非本任务一次做全。
<!-- SECTION:NOTES:END -->
