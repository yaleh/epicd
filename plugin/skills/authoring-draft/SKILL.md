...
Run `bun run cli task view <taskId> --plain` and read the current Description. If
...
```bash
bun run cli task edit <taskId> --append-notes "authoring/draft self-review: <APPROVED|NEEDS_HUMAN> after <n> round(s)<: unresolved criteria if NEEDS_HUMAN>"
```
