...
Run `bun run cli task view <taskId> --plain` and read the Description (the
...
```bash
bun run cli task edit <taskId> --append-notes "authoring/refining review: <APPROVED|NEEDS_HUMAN> after <n> iteration(s)<: unresolved criteria if NEEDS_HUMAN>"
```
