...
describe("computeSequences", () => {
	...
	it("groups all independent tasks into sequence 1", () => {
		...
		const s1: Sequence = mustGet(seqs, 0);
		expect(s1.index).toBe(1);
		expect(s1.tasks.map((t) => t.id)).toEqual(["task-1", "task-2", "task-3"]);
	});
	...
	it("handles a simple chain A -> B -> C", () => {
		...
		expect(mustGet(seqs, 0).tasks.map((t) => t.id)).toEqual(["task-1"]);
		expect(mustGet(seqs, 1).tasks.map((t) => t.id)).toEqual(["task-2"]);
		expect(mustGet(seqs, 2).tasks.map((t) => t.id)).toEqual(["task-3"]);
	});
	...
	it("groups parallel branches (A -> C, B -> C) into same sequence", () => {
		...
		expect(mustGet(seqs, 0).tasks.map((t) => t.id)).toEqual(["task-1", "task-2"]);
		// Second layer contains 3
		expect(mustGet(seqs, 1).tasks.map((t) => t.id)).toEqual(["task-3"]);
	});
	...
	it("handles a more complex graph", () => {
		...
		const seqs = computeSequences(tasks);
		expect(seqs.length).toBe(3);
		expect(mustGet(seqs, 0).tasks.map((t) => t.id)).toEqual(["task-1", "task-2", "task-3"]);
		// Second layer should include 4 and 5 (order by id)
		expect(mustGet(seqs, 1).tasks.map((t) => t.id)).toEqual(["task-4", "task-5"]);
		// Final layer 6
		expect(mustGet(seqs, 2).tasks.map((t) => t.id)).toEqual(["task-6"]);
	});
