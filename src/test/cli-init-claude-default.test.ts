...
describe("init Claude agent default", () => {
	...
	it("does not install Claude agent by default in non-interactive mode", async () => {
		// CLI-CONTRACT: verifies 'epicd init --defaults' does not create Claude agent files unless --install-claude-agent is passed
		...
		const agentExists = await Bun.file(join(TEST_DIR, ".claude", "agents", "project-manager-epicd.md")).exists();
		expect(agentExists).toBe(false);
	});

	it("installs Claude agent when flag is true", async () => {
		// CLI-CONTRACT: verifies 'epicd init --defaults --install-claude-agent true' creates Claude agent file at .claude/agents/
