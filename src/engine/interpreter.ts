...
export type ItemReadyEvent = string; // "item-ready: <pipeline_id>:<phase>:<task_id>"
...
export class Interpreter {
	...
	register(pipeline: Pipeline, phase: string, handler: Handler): void {
		this.pipelines.set(pipeline.id, pipeline);
		this.registry.set(`${pipeline.id}:${phase}`, handler);
	}
	...
	scan(tasks: Task[]): ItemReadyEvent[] {
		...
		for (const task of tasks) {
			...
			const pstate = pipeline.states.find((s) => s.name === task.phase);
			if (pstate?.actor !== "machine") continue;
			events.push(`item-ready: ${task.pipeline_id}:${task.phase}:${task.id}`);
		}
		return events;
	}
	...
	async dispatch(events: ItemReadyEvent[], tasks: Task[]): Promise<void> {
		for (const event of events) {
			// parse: "item-ready: <pipeline_id>:<phase>:<task_id>"
			...
			const handler = this.registry.get(`${pipeline_id}:${phase}`);
			if (handler) {
