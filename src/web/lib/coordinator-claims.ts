...
function readActiveAgentIds(backlogDir: string): Set<string> {
	const activeFile = join(backlogDir, ".active-agents");
...
function readClaimedWorktreePath(backlogDir: string, taskId: string): string | null {
	const wtFile = join(backlogDir, ".caps", `${taskId}.wt`);
...
/**
 * Resolve the claim state for a single task id. Reads `.active-agents` (flat
 * list of currently-claimed task ids) and, when claimed, the recorded
 * `.caps/<taskId>.wt` worktree path — a claim whose worktree no longer exists
 * on disk is "stale" rather than "claimed".
 */
...
/**
 * Batch variant: resolves claim state for every id currently present in
 * `.active-agents` in one pass (avoids re-reading the file per task). Ids not
 * present in `.active-agents` are implicitly "unclaimed" and omitted from the
 * returned map — callers should treat a missing entry as "unclaimed".
 */
